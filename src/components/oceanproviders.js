...
class OceanProviders extends Component {
	...
	componentWillMount() {
		let elem = this;
      	var promise = new Promise(function(resolve, reject) {
			let res;
			if(res = elem.props.providerSelected('ocean')) {
				resolve(res);
			}
			else {
				reject(res);
	...
	render() {
		...
		return (<div className="home-container"><h1 className="heading">{count} Ocean Providers</h1><ul className="providers"> { final } </ul></div>);
	}
}
